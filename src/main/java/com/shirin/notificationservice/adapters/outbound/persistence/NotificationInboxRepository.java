...
public interface NotificationInboxRepository extends JpaRepository<NotificationInboxEntity, UUID> {
  ...
  @Query(
      value = "SELECT status FROM notification_inbox WHERE event_id = :eventId",
      nativeQuery = true)
  ...
  // برای سیستم event-driven، این خیلی مهم است چون duplicate delivery طبیعی است و نمی‌خواهی برنامه
  // با exception دیتابیس spam شود.
  ...
  // @Modifying(clearAutomatically = true, flushAutomatically = true) added to keep persistence
  // context consistent.
  @Query(
      value =
          """
