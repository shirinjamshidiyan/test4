...
public class JpaInboxIdempotencyAdapter implements InboxIdempotencyPort {
  ...
  public AcquireResult acquire(UUID eventId, int maxAttempts) {
    ...
    if (updated == 1) {
      log.debug(
          "InboxIdempotency acquire: incremented attempts (<= {}), eventId={}",
          maxAttempts,
          eventId);
      return AcquireResult.ACQUIRED;
    }
    ...
    log.warn(
        "Inbox acquire: retry exhausted or not eligible, maxAttempts={}, eventId={}",
        maxAttempts,
        eventId);
  ...
  public void markDone(UUID eventId) {
    ...
    if (updated == 1) {
      ...
      log.debug(
          "InboxIdempotency markDone: no update (already DONE or missing row), eventId={}",
          eventId);
    }
  }
  ...
  private InboxStatus readStatus(UUID eventId) {
    ...
    if (raw.isEmpty()) {
      ...
      log.warn(
          "InboxIdempotency acquire: status missing after insert conflict, eventId={}", eventId);
