...
//    FAIL    → header + null → error handler
    ...
    public void onMessage(OrderConfirmedEventV1 event,
        ...
        // End-to-end timer: from message receipt to final outcome (success/duplicate/invalid/failed/exhausted)
        ...
        try (var ctx = LogContext.fillMDC(correlationId, event, topic, partition, offset)) {
            ...
            if (event != null) {
                log.info("OrderConfirmedEventV1 received orderId={} eventId={}", event.orderId(), event.eventId());
            } else {
            ...
            // UseCase already increments some metrics; here we ensure Kafka-level outcome is recorded consistently.
