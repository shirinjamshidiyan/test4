...
public class KafkaDltConfig {
    ...
    //Spring Kafka وقتی از ErrorHandlingDeserializer استفاده می‌کنی، exception و raw bytes را داخل headers می‌گذارد با این نام‌ها
    ...
    ////گر JSON خراب باشد، ErrorHandlingDeserializer یک DeserializationException را serialize می‌کند و داخل header می‌گذارد.
    //این exception شامل getData() است، یعنی raw bytes اصلی پیام.

    //2- Business/processing fail بعد از deserialize موفق
    //عنی object ساخته شده ولی validation یا اجرای use case شکست خورده (مثلا InvalidEventException یا SMTP مشکل دارد).
