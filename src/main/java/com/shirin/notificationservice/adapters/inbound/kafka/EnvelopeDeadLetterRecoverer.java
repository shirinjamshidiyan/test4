...
public class EnvelopeDeadLetterRecoverer extends DeadLetterPublishingRecoverer {
  ...
  public EnvelopeDeadLetterRecoverer(KafkaTemplate<String, DltEnvelope> template, ObjectMapper om) {
    super(template, (record, e) -> new TopicPartition(record.topic() + ".DLT", record.partition()));
  ...
  protected ProducerRecord<Object, Object> createProducerRecord(
      ConsumerRecord<?, ?>
          record, // record : رکورد اصلی که از topic اصلی خوانده شد (topic اصلی، partition، offset،
      // headers اصلی، key/value اصلی به شکل type generic)
      TopicPartition topicPartition, // topicPartition : مقصدی که recoverer تعیین کرده، مثلا
      // order-confirmed.v1.DLT
      Headers headers, // headers : headerهایی که قرار است روی رکورد DLT قرار بگیرد (می‌تواند شامل
      // headerهای اصلی و headerهای خطا باشد)
      @Nullable
          byte[] keyBytes, // key, value : “نمایش bytes” از key/value که recoverer فکر می‌کند باید
      // publish کند.
      @Nullable byte[] valueBytes
      // byte[] keyBytes, byte[] valueBytes
      // نمایش byte-level از key/value که framework در این مرحله دارد.
      // نکته: در deserialization-fail روی این‌ها نمی‌شود کامل حساب کرد، چون raw ممکن است جای دیگری
      // (headers) باشد.
      ) {
    ...
    // Extract DeserializationException from ErrorHandlingDeserializer headers (when JSON
    // deserialize failed)
    ...
    //        String errorStacktrace = extractStringHeader(headers,
    ...
    ProducerRecord<String, DltEnvelope> out =
        new ProducerRecord<>(
            topicPartition.topic(), topicPartition.partition(), null, null, envelope, headers);
