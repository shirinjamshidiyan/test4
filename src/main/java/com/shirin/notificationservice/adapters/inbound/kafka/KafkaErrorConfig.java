...
public class KafkaErrorConfig {
  ...
  public EnvelopeDeadLetterRecoverer deadLetterRecoverer(
    ...
    //        //وقتی retry تمام شد و هنوز fail بود، این recoverer پیام را publish می‌کند به یک topic
    // دیگر (DLT)
  ...
  public DefaultErrorHandler errorHandler(EnvelopeDeadLetterRecoverer recoverer) {
    ...
    //
    //        و معمولاً value را null می‌کند یا یک DeserializationException wrapper تولید می‌کند
    // (بسته به config)
    //
  ...
  //            super(template, (record, ex) -> new TopicPartition(record.topic() + ".DLT",
  // record.partition()));
  //            this.om = om;
  //        }
  ...
  //
  //
  //            String keyStr = keyBytes == null ? null : new String(keyBytes,
  ...
  //                return "{\"error\":\"failed to serialize DLT envelope\",\"message\":\"" +
  // e.getMessage() + "\"}";
  //            }
  //        }
