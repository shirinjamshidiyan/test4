...
public class SendReceiptEmailUseCase {
  ...
  public SendReceiptEmailUseCase(
  ...
  public ProcessingResult handle(OrderConfirmedEventV1 event) {
    ...
    try {
      ...
      try {
        ...
        sendSample.stop(
            metrics.emailSendTimer(
                NotificationMetrics.PROVIDER_SMTP,
                NotificationMetrics.TEMPLATE_RECEIPT,
                "success"));
        metrics.incEmailSendCounter(
            NotificationMetrics.PROVIDER_SMTP, NotificationMetrics.TEMPLATE_RECEIPT, "success");
      } catch (Exception ex) {
        sendSample.stop(
            metrics.emailSendTimer(
                NotificationMetrics.PROVIDER_SMTP,
                ...
                "failure"));
        metrics.incEmailSendCounter(
            NotificationMetrics.PROVIDER_SMTP, NotificationMetrics.TEMPLATE_RECEIPT, "failure");
        throw ex;
      }
